...
async function summarizeIndexArtifact(manager: MemoryManager): Promise<string | null> {
  ...
  let stat: fsSyncStats;
  try {
    stat = await fs.stat(dbPath);
  } catch (err) {
    const code = (err as NodeJS.ErrnoException).code;
    if (code === "ENOENT") {
      throw new Error(`Index file not found: ${shortenHomePath(dbPath)}`, { cause: err });
    }
    throw new Error(
      `Index file check failed: ${shortenHomePath(dbPath)} (${code ?? "error"})`,
      { cause: err },
    );
  }
  if (!stat.isFile() || stat.size <= 0) {
    throw new Error(`Index file is empty: ${shortenHomePath(dbPath)}`);
  }
  return `Index: ${shortenHomePath(dbPath)} (${stat.size} bytes)`;
}
